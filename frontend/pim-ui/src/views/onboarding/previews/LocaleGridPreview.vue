  ...
  <div class="space-y-4">
    ...
    <div v-if="hasContent" class="grid grid-cols-3 gap-2">
      <div class="rounded-lg border border-gray-100 bg-white p-3 text-center">
        ...
        <p class="text-[10px] text-pim-muted">Languages</p>
      </div>
      <div class="rounded-lg border border-gray-100 bg-white p-3 text-center">
        ...
        <p class="text-[10px] text-pim-muted">Auto-Translate</p>
      </div>
      <div class="rounded-lg border border-gray-100 bg-white p-3 text-center">
        <p class="text-lg font-semibold text-amber-600">
          {{ data.default_currency_symbol ?? '—' }}
        </p>
        <p class="text-[10px] text-pim-muted">Currency</p>
      </div>
    </div>

    <!-- Primary Language Card -->
    <div v-if="data.primary_language" class="rounded-xl border border-primary-200 bg-primary-50/50 p-4 shadow-sm">
      <div class="flex items-center gap-3">
        <!-- Flag -->
        <div
          class="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-lg bg-white text-xl shadow-sm"
          v-html="getFlag(data.primary_language)"
        />

        <div class="min-w-0 flex-1">
          <div class="flex items-center gap-2">
            <h3 class="text-sm font-semibold text-pim-text">
              {{ data.primary_language_label ?? data.primary_language }}
            </h3>
            <span class="rounded-full bg-primary-100 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-primary-700">
              Primary
            </span>
          </div>
    ...
    <div v-if="data.additional_languages.length > 0" class="rounded-xl border border-gray-200 bg-white shadow-sm">
      ...
      <div class="grid grid-cols-2 gap-px bg-gray-100 p-px">
        <div
          v-for="lang in data.additional_languages"
          :key="lang.code"
          class="flex items-center gap-2.5 bg-white p-3"
        >
          <!-- Flag -->
          ...
          <div class="min-w-0 flex-1">
            ...
            <p class="text-[10px] uppercase text-pim-muted">
              {{ lang.code }}
            </p>
          </div>
    ...
    >
      ...
      <div class="space-y-2.5">
        ...
        <div v-if="data.default_currency" class="flex items-center gap-2">
          <div class="flex h-7 w-7 flex-shrink-0 items-center justify-center rounded bg-amber-100">
          ...
          <div>
            <p class="text-xs font-medium text-pim-text">
              {{ data.default_currency }}
            </p>
            <p class="text-[10px] text-pim-muted">Default Currency</p>
          </div>
        </div>

        <!-- UOM -->
        <div v-if="data.default_uom" class="flex items-center gap-2">
          <div class="flex h-7 w-7 flex-shrink-0 items-center justify-center rounded bg-slate-100">
          ...
          <div>
            ...
            <p class="text-[10px] text-pim-muted">Default Unit of Measure</p>
          </div>
        </div>
      </div>
    </div>
    ...
    <div
      ...
      class="flex items-center gap-2 rounded-lg border border-green-100 bg-green-50 p-3"
    >
    ...
    >
      ...
      <p class="text-sm text-pim-muted">Configure localization to see language settings</p>
    </div>
  </div>
</template>
