    ...
    <template v-if="!props.reviewMode">
      ...
      <div class="rounded-lg border border-primary-200 bg-primary-50 p-4">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h3 class="font-medium text-primary-900 capitalize">{{ archetypeName }} Template</h3>
            <p class="text-sm text-primary-700">Ready to apply to your workspace</p>
          </div>
        </div>
      </div>
      ...
      <div v-if="preview" class="space-y-3">
        <h3 class="text-sm font-medium text-pim-text">This template will create:</h3>
        <div class="grid grid-cols-2 gap-3 sm:grid-cols-3">
          <div
            v-for="(section, key) in preview.sections"
            :key="key"
            class="rounded-lg border border-pim-border bg-white p-3 text-center"
          >
            <p class="text-2xl font-bold text-primary-600">{{ section.count }}</p>
            <p class="mt-0.5 text-xs text-pim-muted capitalize">{{ String(key).replace(/_/g, ' ') }}</p>
          </div>
        </div>
      </div>
      ...
      <div v-if="result" class="space-y-3">
        ...
        <div
          ...
          :class="{
            'border border-green-200 bg-green-50': isSuccess,
            'border border-red-200 bg-red-50': !isSuccess,
          }"
        >
          <svg
            v-if="isSuccess"
          ...
          <div>
            <p class="font-medium" :class="isSuccess ? 'text-green-900' : 'text-red-900'">
              {{ isSuccess ? 'Template applied successfully!' : 'Template application failed' }}
            </p>
            <p class="text-sm" :class="isSuccess ? 'text-green-700' : 'text-red-700'">
        ...
        <!-- Detail Breakdown -->
        <div v-if="result.details" class="rounded-lg border border-pim-border">
          <div
          ...
          >
            ...
            <div class="flex gap-3 text-xs">
              ...
              <span v-if="detail.skipped" class="text-pim-muted">{{ detail.skipped }} skipped</span>
        ...
        <!-- Errors -->
        <div v-if="result.errors.length > 0" class="rounded-lg border border-red-200 bg-red-50 p-3">
      ...
      <div class="space-y-4">
        <div class="rounded-lg border border-green-200 bg-green-50 p-4">
          <div class="flex items-center gap-3">
            ...
            <div>
              <h3 class="font-medium text-green-900">Your workspace is configured</h3>
        ...
        <div v-if="result" class="rounded-lg border border-pim-border">
